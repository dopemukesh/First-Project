...
const JobCard = ({
    ...
    return (
        <div className={`${bgColors} text-black min-h-[250px] flex flex-col justify-between p-4 group`}>
            ...
            <div className={`relative overflow-hidden p-4 rounded-xl group-hover:-translate-y-8 transition-transform duration-700 ${getCardColor(job.type, cardColors)}`}>
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                    {job.type && <span className={badgeClasses}>{job.type}</span>}
                    {job.level && <span className={badgeClasses}>{job.level}</span>}
